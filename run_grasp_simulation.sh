...
GRASP_TARGET_POINT_CLOUD="${GRASP_TARGET_POINT_CLOUD:-}"
GRASP_TARGET_POINT_CLOUD_FRAME="${GRASP_TARGET_POINT_CLOUD_FRAME:-world}"
GRASP_TARGET_POINT_CLOUD_SOURCE="${GRASP_TARGET_POINT_CLOUD_SOURCE:-}"
...
if [[ -n "$GRASP_TARGET_POINT_CLOUD" ]] && ! has_arg "--target-point-cloud"; then
  CMD+=(--target-point-cloud "$GRASP_TARGET_POINT_CLOUD")
fi

if [[ -n "$GRASP_TARGET_POINT_CLOUD" ]] && ! has_arg "--target-point-cloud-frame"; then
  CMD+=(--target-point-cloud-frame "$GRASP_TARGET_POINT_CLOUD_FRAME")
fi

if [[ -n "$GRASP_TARGET_POINT_CLOUD_SOURCE" ]] && ! has_arg "--target-point-cloud-source"; then
  CMD+=(--target-point-cloud-source "$GRASP_TARGET_POINT_CLOUD_SOURCE")
fi
...
echo "  target_point_cloud=${GRASP_TARGET_POINT_CLOUD:-<from args/fallback>}"
echo "  video=<disabled>"
