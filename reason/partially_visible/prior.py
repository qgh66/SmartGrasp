...
def compute_semantic_prior_all_ancestors(
    ...
    if len(ancestor_mids)<=1:
        return {mid: 1.0 for mid in ancestor_mids}
    ...
    occluders = [
        {"mid": mid, "label": perception.node_info[
    ...
    if labeled_rgb is None:
        ...
        return {mid: 0.5 for mid in ancestor_mids}
...
def compute_semantic_prior(
    ...
    return {mid: all_scores.get(mid, 0.5) for mid in occluder_mids}